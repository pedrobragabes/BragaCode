...
export const metadata: Metadata = createMetadata({
  ...
  description: "WooCommerce case with Python/Docker inventory and pricing synchronization, an internal barcode app and customer-service automation.",
...
export default function EnglishAquafloraCasePage() {
  ...
  return (
    <div lang="en">
      <JsonLd data={jsonLd} />
      <article>
        <header className="case-hero"><div className="container"><Link className="back-link" href="/en/projects">← All projects</Link><div className="case-hero-grid"><div><p className="eyebrow"><span aria-hidden="true" />{project.eyebrow}</p><h1>{project.name}</h1><p className="case-summary">{project.summary}</p><div className="case-meta"><div><span>Status</span><strong>{project.status}</strong></div><div><span>Period</span><strong>{project.period}</strong></div><div><span>Nature</span><strong>Professional work</strong></div></div></div><ProjectVisual kind="sync" label="Reconstructed representation of the AquaFlora synchronization workflow" locale="en" /></div></div></header>

        <section className="case-metrics"><div className="container case-metrics-grid">{project.metrics.map((metric) => <div key={metric.value}><strong>{metric.value}</strong><span>{metric.label}</span><small>{metric.note}</small></div>)}</div></section>
